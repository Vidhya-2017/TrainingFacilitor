...
class BatchFormation extends React.Component{
    ...
    this.props.getTrainingList().then((response) => {
         if (response && response.arrRes) {  console.log("--Response--",response);
            ...
            this.props.getCandidateMapList(reqObj).then((response) => {
                if (response && response.errCode === 200){ 
                    ...
                    let assignedCan = candidateList.filter(batch => batch.batch_id > 0);
                     ...
                     console.log("--Response--",assignedCan.length);
                }
            });
    ...
    render() {
        const { eventSelected,EventDetailsList, batchSelected, batchDetailsList, showUserModal, newBatchName, newBatchCount , candidateList } = this.state;
        this.CandidateIDs = [];
            ...
            <section className="blue_theme">
                <Container>
                    ...
                    <Col xs ={12} md = {12} lg="12" >
                        <h2 className="text-center">Batch Formation</h2>                   
                    </Col>
                    </Row>
                    ...
                    <SelectOne
                        ...
                        id="batch"      
                    /> <i onClick={this.createBatchName} className="addUser fa fa-plus"></i>
                    </Col>
                    {eventSelected && <Col>
                        <Buttons
